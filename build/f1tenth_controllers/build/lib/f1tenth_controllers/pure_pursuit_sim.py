...
class PurePursuitController(Node):
    ...
    def search_target_index(self):
        ...
        if self.current_index is None:
            ...
            distance_this_index = self.calc_distance(self.cx[ind],
                                                      self.cy[ind])
            while True:
                distance_next_index = self.calc_distance(self.cx[(ind + 1)%len(self.cx)],
        ...
        Lf = k * self.current_velocity + Lfc  # update look ahead distance
    ...
    def pure_pursuit_steer_control(self, pind):
        ...
        r = math.sqrt((tx-self.x)**2 + (ty - self.y)**2)

        alpha = math.atan2(ty - self.rear_y, tx - self.rear_x) - self.yaw

        # beta = math.atan2((lr*math.tanh(self.current_servo_position))/WB)
        delta = math.atan2(2.0 * WB * math.sin(alpha) / Lf, 1.0)

        # delta = math.atan2((alpha * WB)/(self.cv[self.target_index] * math.cos(beta)), 1.0)
    ...
    def drive(self):  # main controller function for the robot
        ...
        self.get_logger().info(f'X transformation: {self.x}')
        self.get_logger().info(f'Y transformation: {self.y}')
        ...
        yaw_test = tf_transformations.euler_from_quaternion(
            [
                transform.transform.rotation.x,
                transform.transform.rotation.y,
                transform.transform.rotation.z,
        ...
        test = tf_transformations.euler_matrix(yaw_test[0], yaw_test[1], yaw_test[2])
        self.get_logger().info(f'Euler Matrix: {test}')

        rotation_matrix = test[:3, :3]
        ...
        steering_angle, self.target_index = self.pure_pursuit_steer_control(self.target_index)

        
    #  + [self.x, self.y, self.z]
        lookahead = (rotation_matrix.T @ [self.cx[self.target_index], self.cy[self.target_index], 0.0])
        self.get_logger().info(f'Lookahead: {lookahead}')
        test[0,3] = self.x
        test[1,3] = self.y
        test[2,3] = self.z
        lookahead_trans = (test.T @ [self.cx[self.target_index], self.cy[self.target_index], 0.0, 1.0])
        self.get_logger().info(f'Lookahead_TRANSFORMATION: {lookahead_trans}')

        # Calculate steering angle and set velocity
        velocity = self.cv[self.target_index] * throttle_percentage
        
        # lin_cntrl = self.proportional_control(1.0, velocity, self.current_velocity)
        # angular_cntrl = self.proportional_control(1.0, steering_angle, self.current_servo_position)
