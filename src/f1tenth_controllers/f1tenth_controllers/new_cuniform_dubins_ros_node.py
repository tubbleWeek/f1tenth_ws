...
class CUniform_Numba(object):
  ...
  def get_rollout_cost(self):
    ...
    # transformed_trajectories = self.get_state_rollout(self.x0, self.trajectories) #RUNTIME: significant, ~0.05
    # self.trajectories_d = numba_cuda.to_device(transformed_trajectories.astype(np.float32))#RUNTIME: negligible ~0.002s
    self.u_cur_d = numba_cuda.to_device(self.u_seq0)  #RUNTIME: negligible ~0.002s
  ...
  def get_state_rollout(self, x_curr, trajectories): #RUNTIME: this function has runtime issue, 1000 4s traj take around 0.04-0.06 seconds
  ...
  def rollouts_cost_numba(
    ...
    gamma = 1 # Discount factor for cost
  ...
  def update_useq_numba(
    ...
    for t in range(timesteps):
      for i in range(starti, endi):
        ...
        numba_cuda.atomic.add(u_cur_d, (t, 1), weights_d[i]*trajectories_d[i, t, 3])
    numba_cuda.syncthreads()
...
class CUniformPlannerNode(Node):
    def __init__(self):
        ...
        # self.original_trajectories = cuniform_trajectories_transformed[:,:16,:]
    ...
    def costmap_callback(self, msg: OccupancyGrid):
        ...
        # costmap_filled = np.full((height, width), 100, dtype=np.float32)
    ...
    def solve_cuniform(self):
        try:
            ...
            self.cuniform.shift_and_update(x_current, self.original_trajectories) #RUNTIME: this part is not bottleneck, 1000 4s traj only take around 0.00x seconds

            # Solve CUniform
            self.cuniform.setup(self.cuniform_params)
            start_time = time.time()
            min_idx, _ , _, useq_numba  = self.cuniform.solve()
            ...
            if self.i > 3:
              ...
              if ((self.i % 10) == 0): 
                ...
                self.get_logger().info(f"time to take solve cuniform is {time.time() - start_time}")
              ...
              # else:   
              steering_angle = 0.68*np.tan(u_execute[1]*0.9)*(self.cuniform_params['vehicle_wheelbase'])
              # drive = AckermannDrive(steering_angle=1.0*(math.atan2(u_execute[1]*self.cuniform_params['vehicle_wheelbase'], u_execute[0])), speed=1.0)
              # drive = AckermannDrive(steering_angle=0.8*(np.tan(u_execute[1]*1.0)*(self.cuniform_params['vehicle_wheelbase'])), speed=1.0)
              drive = AckermannDrive(steering_angle=steering_angle, speed=1.0)
              data = AckermannDriveStamped(header=h, drive=drive)

              if ((self.i % 10) == 0): 
                self.get_logger().info(f"Input given: velocity {u_execute[0]}, Steering_Angle: {steering_angle}" )
