...
class PurePursuitController(Node):
    def __init__(self):
        ...
        self.transformation_matrix = self.inv_transformation_matix = np.zeros((3,3))
    ...
    def get_transform_matrix(self, translation, rotation):
        rotation_temp = R.from_quat(rotation)
        transform_matrix = rotation_temp.as_matrix()
        transform_matrix[:2, 2] = [translation[0], translation[1]]
    ...
    def pure_pursuit_steer_control(self, pind):
        ...
        look_ahead_point_robot = np.dot(self.inv_transformation_matix, 
                                        np.array([look_ahead_point_global[0], look_ahead_point_global[1], 1]))

        L2 = look_ahead_point_robot[0]**2 + look_ahead_point_robot[1]**2

        y = look_ahead_point_robot[1]

        delta = k * 2.0 * y / L2 # Angle calculated using CL2 Waterloo p-controllerequation https://github.com/CL2-UWaterloo/f1tenth_ws/blob/main/src/pure_pursuit/src/pure_pursuit.cpp
        # if self.i % 20 == 0:
        self.get_logger().info(f'Lookahead point Robot: {look_ahead_point_robot}')
        self.get_logger().info(f'Delta: {delta}, ind: {ind}, Lookahead Distance: {L2}')
        return delta, ind
    ...
    def drive(self):  # main controller function for the robot
        ...
        current_translation = [transform.transform.translation.x, 
                               transform.transform.translation.y,
                               transform.transform.translation.z]
        current_rotation = [transform.transform.rotation.x,
                transform.transform.rotation.y,
                transform.transform.rotation.z,
                transform.transform.rotation.w]


        self.transformation_matrix = self.get_transform_matrix(current_translation, current_rotation)
        self.inv_transformation_matix = np.linalg.inv(self.transformation_matrix)
